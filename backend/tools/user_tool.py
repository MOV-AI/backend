...
class ConvertOldUser(BaseCommand):
    ...
    @classmethod
    def define_arguments(cls, subparsers) -> None:
        convert_parser = subparsers.add_parser(CONVERT_COMMAND,
                                               help="Converts an old User to the new InternalUser")
        convert_parser.add_argument('-u',
                                    dest='username',
                                    metavar='<USERNAME>',                                    
                                    type=str,
...
class RemoveOldUser(BaseCommand):
    ...
    @classmethod
    def define_arguments(cls, subparsers) -> None:
        remove_parser = subparsers.add_parser(REMOVE_COMMAND,
                                            help="Removes an existing old User")
        remove_parser.add_argument('-u',
                                    dest='username',
                                    metavar='<USERNAME>',                                    
    ...
    def execute(self):
        ...
        if not InternalUser.is_exist(INTERNAL_DOMAIN, self.username):
            if not Role.is_exist(role):
                ...
                Role.create_default_role()
        InternalUser.create(account_name=self.username,
                            password=password,
                            roles=[role],
