...
from endpoints.api.v2.models.ldapconfig import LdapConfig
...
class LDAPConnectionBuilder:
    ...
    def __init__(self, ldap_config: LdapConfig) -> None:
        ...
        self._servers = []
        self._pool = ServerPool(pool_strategy=FIRST, active=3, exhaust=True)
        self._tls = Tls()
        self._connection = None
        set_config_parameter("POOLING_LOOP_TIMEOUT", LDAP_POOLING_LOOP_TIMEOUT)
    ...
    def create_server(self, host: str, port: str) -> None:
        ...
        if isinstance(host, str) and host != "":
            ...
            if server not in self._pool.servers:
                ...
                LOGGER.warning(f"server {server} is already in server pool")
...
class LDAPHandler(LDAPBaseClass):
    ...
    def __init__(self, domain_name: str) -> None:
        ...
        """
        LOGGER.debug(f"loading LdapConfig: {domain_name}")
    ...
    def validate_configuration(self) -> bool:
        ...
        try:
            ...
            LOGGER.info(f"Succesfully validated {self._domain_name} LDAP "
                    "Configuration.")
        except LDAPException:
            status = False
            self._ldap_config.update_validation(status)
            LOGGER.warning(f"Failed to validate {self._domain_name} LDAP "
                    "Configuration.")
        finally:
            return status
    ...
    def authenticate_user(self, username: str, password: str) -> bool:
        ...
        try:
            ...
            LOGGER.warning(
                ...
                f" {self._domain_name}")
            LOGGER.warning(f"details: {e.description}")
        return False
    ...
    def get_object_info(self,
        ...
        self.ldap_connention.bind()
        LOGGER.debug("LDAP connection has successfully bound user"
                     f" {self._ldap_config.username} with domain"
                     f" {self._ldap_config.domain_name}")
        LOGGER.debug(f"""querying LDAP server:
    ...
    def search_distinguished_name(self,
        ...
        if self.ldap_connention.bind():
            ...
            try:
                ...
                LOGGER.warning("ldap query did not return any results")
                return []
            except LDAPException as ldap_exception:
                LOGGER.warning(ldap_exception)
                return []
...
class LDAPObject(LDAPBaseClass):
    ...
    def display_info(self):
        ...
        """
        self.check_initialization()
        LOGGER.info(f"distinguished_name: {self.distinguished_name}")
        LOGGER.info(f"account_name: {self.account_name}")
        LOGGER.info(f"common_name: {self.common_name}")
        LOGGER.info(f"category: {self.category}")
        LOGGER.info(f"sid: {self.sid}")
...
class LDAPUser(LDAPObject):
    ...
    def display_info(self) -> None:
        ...
        self.check_initialization()
        LOGGER.info(f"account_name: {self.account_name}")
        LOGGER.info(f"display_name: {self.display_name}")
        LOGGER.info(f"principal_name: {self.principal_name}")
        LOGGER.info(f"member_of: {self.member_of}")
