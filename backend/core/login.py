...
class AuthenticationManager:
    ...
    def __init__(self, auto_init: bool = True) -> None:
        ...
        """
        self.domain_list = [INTERNAL_DOMAIN, HOST_DOMAIN]
        self.infs = {}
        if auto_init:
            self._auto_init()
    ...
    def _auto_init(self) -> None:
        ...
        for domain in self.domain_list[2:]:
            self.register_authenticator(domain, LDAPAuthentication(domain))

    def _init_ldap_domains(self) -> None:
        """helper function to query all ldap configurations from DB"""
        for domain_name in LdapConfig.list_config_names():
            LOGGER.debug(f"appending ldap domain: {domain_name}")
