...
from movai_core_shared.envvars import INTERNAL_DOMAIN
    ...
    def extract_object(self) -> None:
        ...
        if self._object_name:
            try:
                ...
                error_msg = f"The object  {self._scope_name}:"\
...
class PostScope(GetScope):
    ...
    async def execute_imp(self) -> None:
        ...
        user_obj = InternalUser.create(data['AccountName'],
                                       data['Password'],
                                       data['Roles'],
                                       data['CommonName'],
                                       data['Email'],
...
class ResetPassword(InternalUserRestBaseClass):
    ...
    def validate_superuser_permission(self) -> None:
        """Validates that the requesting user has super_user flag turned on.

        Raises:
            UserPermissionsError: if the super_user flag is turned off.
        """
        if not self._user.super_user:
            error_msg = "User must have super_user flag for password reset."
            raise UserPermissionsError(error_msg)
    ...
    async def execute_imp(self) -> None:
        ...
        """
        self.extract_object()
        self.validate_superuser_permission()
        data = await self._request.json()
        self._object.reset_password(data["NewPassword"],
...
class ChangePassword(ResetPassword):
    ...
    async def execute_imp(self) -> None:
        ...
        data = await self._request.json()
        self._user.change_password(data["CurrentPassword"],
                                   data["NewPassword"],
                                   data["ConfirmPassword"])
