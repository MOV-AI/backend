from movai_core_shared.exceptions import (AclObjectDoesNotExist,
                                          AclObjectError,
                                          RestrictedPathError)
...
class AclObjectRestBaseClass(RestBaseClass, ABC):
    ...
    def query_scope(self) -> None:
        ...
        for key in self._result.keys():
            if self._result[key]['ObjectType'] != self._obj_type or\
               self._result[key]['DomainName'] != self._domain_name:
...
class SearchDomainObjects(GetAclObject):
    ...
    async def execute_imp(self) -> None:
        ...
        self._result = AUTH_MANAGER.search_objects(self._domain_name,
                                                   common_name,
                                                   self._obj_type)
        ...
        for acl_object in acl_objects:
            try:
                ...
                if 'Roles' in acl_object:
                    self.validate_role(acl_object['Roles'])
                obj = AclObject.get_object_by_name(self._domain_name,
                                                   account_name)
    ...
    async def execute_imp(self) -> None:
        ...
        for acl_object in acl_objects:
            try:
                account_name = acl_object['AccountName']
                self._result[account_name] = {}
                AclObject.remove(self._domain_name,
                                 acl_object['AccountName'],
                                 acl_object['ID'])
                self._result[account_name]['success'] = True
            except AclObjectError as error:
                self._result[account_name]['success'] = False
...
class AclAPI(BaseWebApp):
    ...
    def routes(self) -> List[web.RouteDef]:
        ...
        """
        return [web.get(rf"/{self.multiple_object_path}/search",
                        SearchDomainObjects()),
