...
class RestV1App(IWebApp):
    ...
    def routes(self) -> List[web.RouteDef]:
        ...
        return [
            web.post(r'/User/{name}/reset-password/',
                     self._rest_api.post_reset_password),
            web.post(r'/User/change-password/',
                     self._rest_api.post_change_password),
            web.post(r'/newUser/', self._rest_api.new_user),
            web.post(r'/upload/{package_name}/',
                     self._rest_api.upload_static_file),
            ...
            web.get(r'/database/{scope:(global|fleet)}/{key}/',
                    self._rest_api.get_key_value),
            web.post(r'/database/', self._rest_api.set_key_value),
            web.delete(
                r'/database/{scope:(global|fleet)}/{key}/', self._rest_api.delete_key_value),
            ...
            web.get(address_format %
                    REST_SCOPES, self._rest_api.get_scope),
            web.post(address_format %
                     REST_SCOPES, self._rest_api.post_to_scope),
            web.put(address_format %
                    REST_SCOPES, self._rest_api.add_to_scope),
            web.delete(address_format %
                       REST_SCOPES, self._rest_api.delete_in_scope),
            web.get(r'/{scope:%s}/' % REST_SCOPES, self._rest_api.get_scope),
            web.post(r'/{scope:%s}/' % REST_SCOPES,
                     self._rest_api.post_to_scope)
        ]
