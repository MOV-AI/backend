...
try:
    ...
    print("Caught exception while loading movai 2.4.0>=  libs", e)
    print("Importing older version as fallback")
    from API2.GraphicScene import GraphicScene
    from API2.Package import Package
    from movai.data.scope import ScopesTree
    from deprecated.api.models.var import Var as Variable
...
def get(obj, key):
    return lambda or_else: obj[key] if key in obj else or_else
...
def polygon2redis(polygon, tree_object):
    ...
    z_points = list(map(lambda x: complex(x[0], x[1]),
...
def graph2redis(graph, tree_object):
    ans_graph = {
        ...
        "graph": {
            ...
            "annotationPropOverrides": get(graph,
...
def add2scene_aux(tree_node, scene_name, tree_object, scene):
    ...
    try:
        sprint("Add GraphicScene AssetType and AssetName", encode_obj_type,
               encode_key)
    ...
    # Add annotation
    add_annotation_to_object(scene, tree_node, encode_obj_type, encode_key,
                             lambda obj: obj.Annotation, "Annotation",
                             "keyValueMap")
    add_annotation_to_object(scene, tree_node, encode_obj_type, encode_key,
                             lambda obj: obj.AnnotationOverride,
                             "AnnotationOverride", "annotationPropOverrides")
...
def add_annotation_to_object(scene,
                             tree_node,
                             obj_type,
                             asset_name,
                             prop_name_getter,
...
def del2scene_aux(obj_name, scene_name, scene_scope):
    ...
    try:
        ...
        if node:
            ...
            sprint("Deleting GraphicScene, AssetType and AssetName",
                   encode(node_type), encode(obj_name))
            del scene_scope.AssetType[encode(node_type)].AssetName[encode(
                obj_name)]
...
def validate_configuration(value):
    """
    Validate configuration value and keys
    """
    try:
...
class TreeObject:
    ...
    def get_parent_node_recursive(tree, predicate, parent):
        for node in tree:
            if predicate(node):
                return parent
            answer = TreeObject.get_parent_node_recursive(
                node["children"], predicate, node)
            if answer:
                return answer
        return None
        ...
        for node in tree:
            ...
            if "children" in node:
                for child in node["children"]:
                    child_result = TreeObject.find_path_recursive([child],
                                                                  predicate)
    ...
    def get_parent_node(self, child_predicate):
        if self.tree:
            return TreeObject.get_parent_node_recursive(
                self.tree, child_predicate, None)
        return None
...
# callbacks
def on_save(tree, scene_name=DEFAULT_SCENE_NAME):
    ...
    def update_tree():
        ...
        for leaf in tree:
            ...
            on_addNodeItem(leaf, None, scene_name)
...
def on_addNodeItem(tree_node, parent_name, scene_name=DEFAULT_SCENE_NAME):
    ...
    if parent_name:
        ...
        ans = list(
            filter(lambda node: tree_node["name"] != node["name"], tree.tree))
...
def on_deleteNodeByName(node_name, scene_name=DEFAULT_SCENE_NAME):
    ...
    if parent_node:
        parent_node["children"] = list(
            filter(lambda child: child["name"] != node_name,
                   parent_node["children"]))
    else:
        tree.tree = list(
            filter(lambda node: node_name != node["name"], tree.tree))

    MemorySingleton.set_tree(tree.tree, scene_name)


def on_updateNode(tree_node,
                  parent_name,
                  old_name=None,
    ...
    retrieved_node = TreeObject(get_tree(scene_name)).get_node(
        ...
        if old_name is not None else node["name"] == tree_node["name"])
    if retrieved_node:
...
def migrate_poses_in_scene(scene_path):
    try:
        types = [
            'KeyPoint', 'Map', 'Mesh', 'GlobalRef', 'Robot', 'PointCloud',
            'NodeItem'
        ]
...
def get_computed_annotations(annotations_name):
    ...
    for annotation_name in annotations_name:
        try:
            annotation = ScopesTree().from_path(annotation_name,
                                                scope="Annotation")
            computed_annotations_dict.update(
                annotation.get_computed_annotation())
        except Exception as e:
            sprint("Caught Exception while computing annotation",
                   annotation_name, e)
...
def set_robot_pose_estimation(robot_name, pose, params):
    ...
    Variable("Fleet", _robot_name=robot_name).set(f"pose_estimation", {
        "pose": pose,
        "params": params
    })
...
key2action_map = {
    ...
    "deletePointCloud": on_delete_point_cloud,
    "validateConfiguration": validate_configuration,
    "getComputedAnnotations": get_computed_annotations,
    "setRobotMesh": set_robot_mesh,
    "setRobotPoseEstimation": set_robot_pose_estimation,
    "setRobotPoseGoal": set_robot_pose_goal,
}
